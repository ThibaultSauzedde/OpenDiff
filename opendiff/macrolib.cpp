...
namespace mat
{
    ...
    Macrolib::Macrolib(const mat::Materials &materials, const std::vector<std::vector<std::vector<std::string>>> &geometry)
    {
        ...
        Eigen::Tensor<std::string, 3> geometry_tensor(geometry.size(), geometry[0].size(), geometry[0][0].size());
    ...
    const Eigen::Tensor<double, 3>& Macrolib::getValues1DView(const int i_grp, const std::string &reac_name) const
    {
        Eigen::Tensor<double, 3> values = getValues(i_grp, reac_name);

        Eigen::array<Eigen::DenseIndex, 1> one_dim({values.dimension(0)*values.dimension(1)*values.dimension(2)});

        // std::cout << one_dim << std::endl;

        Eigen::Tensor<double, 1> values_1d = values.reshape(one_dim);

        std::cout << values_1d << std::endl;
    ...
    const py::array_t<double>& Macrolib::getValues1DViewPython(const int i_grp, const std::string &reac_name) const
    {
        std::cout << "test0" << std::endl;

        auto values_1d = getValues1DView(i_grp, reac_name);
